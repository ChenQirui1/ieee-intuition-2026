...
from database import page_id_for_url, simplification_id_for
...
def simplify(req: SimplifyRequest):
    ...
    for mode in wanted_modes:
        if not req.force_regen:
            # Use database abstraction to find cached simplification
        ...
        # Use database abstraction to save simplification
...
def firestore_test():
    ...
    # This endpoint is Firebase-specific, so we check if using Firebase
    if hasattr(db, 'db'):
